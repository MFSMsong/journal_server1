...
public class ActivityController {
    ...
    Result getActivityMembers(@PathVariable("activityId") String activityId, @UserId String userId) {
        Activity query = Activity.builder().activityId(activityId).userId(userId).build();
        if (!service.hasQueryPermission(query)) {
            return Result.error(ResultStatus.NOT_OWN_RESOURCE);
        }
        List<ActivityMember> members = service.queryActivityMembers(activityId);
        return Result.ok(members);
    }

    @Authorization
    @PostMapping("/nickname")
    Result updateNickname(@RequestBody ActivityUserRel activityUserRel, @UserId String userId) {
        String activityId = activityUserRel.getActivityId();
        Activity query = Activity.builder().activityId(activityId).userId(userId).build();
        if (!service.hasQueryPermission(query)) {
            return Result.error(ResultStatus.NOT_OWN_RESOURCE);
        }
        
        String targetUserId = activityUserRel.getTargetUserId();
        ...
        if (targetUserId != null && !targetUserId.isEmpty()) {
            ...
            activityUserRel.setUserId(userId);
        }
    ...
    @Authorization
    @DeleteMapping("/kick/{activityId}/{userId}")
    Result kickMember(@PathVariable("activityId") String activityId, 
                      @PathVariable("userId") String targetUserId,
                      @UserId String userId) {
        Activity query = Activity.builder().activityId(activityId).userId(userId).build();
        if (!service.isOwnerActivity(query)) {
            return Result.error(ResultStatus.NOT_OWN_RESOURCE, "只有创建者可以移除成员");
        }
        
        Activity activity = service.queryActivityByActivityId(activityId);
        if (activity != null && activity.getUserId().equals(targetUserId)) {
